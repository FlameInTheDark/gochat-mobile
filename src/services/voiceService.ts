...
async function requestCameraStream(deviceId: string): Promise<MediaStream> {
  ...
  const preferredVideo: MediaTrackConstraints | boolean = deviceId
    ? { deviceId: { exact: deviceId } }
    : nativeMobile ? { facingMode: 'user' } : true

  try {
    return await navigator.mediaDevices.getUserMedia({ video: preferredVideo, audio: false })
  } catch (error) {
    if (!nativeMobile || !deviceId) {
      throw error
    }

    vwarn('requestCameraStream: exact mobile device failed, retrying front camera — %s', (error as Error).message)
...
export async function enableCamera(): Promise<void> {
  ...
  try {
    nextVideoStream = await requestCameraStream(videoInputDevice)
  } catch (err) {
  ...
  try {
    ...
    stopMediaTracks(nextVideoStream)
    throw new Error('Failed to enable camera in this voice call.')
  }
