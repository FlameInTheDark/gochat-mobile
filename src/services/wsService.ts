...
import { isAppActive, showNativeLocalNotification } from '@/services/nativeNotifications'
 ...
 */
async function refreshTokenAndReconnect(options: { immediate?: boolean } = {}) {
          ...
          if (!isChannelVisible(channelId)) {
            ...
            if (!isAppActive()) {
              void showNativeLocalNotification({
                title: notif.from?.name ?? 'GoChat',
                body: 'New direct message',
                data: {
                  type: 'dm_message',
                  userId: notif.from?.id,
                  channelId,
...
function onClose() {
  ...
  if (!authSucceeded) {
    ...
    if (wasQuickClose || openTime === null) {
      ...
      void refreshTokenAndReconnect({ immediate: true })
      return
    }
  }
...
export function resumeWebSocketFromBackground() {
  if (intentionalClose || !currentToken) return

  const heartbeatStale =
    lastHeartbeatAt !== 0 &&
    Date.now() - lastHeartbeatAt > Math.max(45_000, heartbeatIntervalMs * 2)

  if (socket?.readyState === WebSocket.OPEN && !heartbeatStale) {
    dispatchHeartbeat()
    return
  }
